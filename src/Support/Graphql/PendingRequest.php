...
class PendingRequest
{
    ...
    /**
     * Issue a query to the graphql endpoint.
     *
     * @param  string  $query
    ...
    public function query(string $query = '', array $variables = [])
    {
        ...
        if (!empty($data)) {
            return $this->send([RequestOptions::JSON => $data]);
        } else {
            return $this->send();
        }
    }
    ...
    {
        $url = ltrim(rtrim($this->baseUrl, '/'), '/');

        return retry($this->tries ?? 1, function () use ($url, $options) {
            try {
                return tap(new Response($this->buildClient()->request('POST', $url, $this->mergeOptions([
    ...
    public function runBeforeSendingCallbacks($request, array $options)
    {
        return tap($request, function ($request) use ($options) {
            $this->beforeSendingCallbacks->each->__invoke((new Request($request))->withData($options['laravel_data']), $options);
        });
    }
    ...
    public function buildStubHandler()
    {
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? collect())->map->__invoke((new Request($request))->withData($options['laravel_data']), $options)->filter()->first();
